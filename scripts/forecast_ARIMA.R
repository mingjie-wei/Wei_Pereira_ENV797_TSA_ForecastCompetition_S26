# Model: seasonal ARIMA via auto.arima() with weekly seasonality (m = 7).
# From repo root: Rscript scripts/forecast_ARIMA.R
...
out_csv <- file.path(out_dir, "submission_ARIMA.csv")
...
fit_and_forecast <- function(y_numeric, h, label) {
  # auto.arima() uses one seasonal period; daily load: weekly cycle (m = 7).
  y_ts <- stats::ts(y_numeric, frequency = 7)
  ...
  message("Model: seasonal ARIMA (", label, "): ", format(fit))
